...
impl ChatSession {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        loop {
            ...
            match command {
                ...
                ChatCommand::Explain => self.handle_explain().await?,
    ...
    async fn handle_explain(&mut self) -> Result<()> {
        renderer::render_system_message("Preparing a conceptual explanation...");

        let task_body = self.task_description.as_ref().map(|d| d.body.as_str());
        let prog_lang = self.config.general.programming_language.clone();

        // Try to attach current code as context (optional — explain works without it too)
        let code_pair: Option<(String, String)> = if let Some(ref dir) = self.task_dir {
            code_reader::read_solution(dir)
                .ok()
                .flatten()
        ...
        let prompt = prompts::get_explain_prompt(task_body, code_ref, &prog_lang);
        ...
        self.save_to_history("user", "[Conceptual explanation requested]")?;
        self.save_to_history("assistant", &response)?;
